...
public class CautiousShooter : Agent
{
    ...
    public override void OnEpisodeBegin()
    {
        _history.Clear();
        _hasSeenEnemy = false;
        _lastEnemyPosition = Vector3.zero;
        _stationaryTimer = 0f;
        _moveInput = 0f;
        _strafeInput = 0f;
        _turnInput = 0f;
        ...
        if (eyeRaySensor != null && _shootInput > 0.5f)
        {
            ...
            foreach (var ray in output.RayOutputs)
            {
                if (ray.HitTagIndex == 1) 
                {
                    ...
                    averageDirection += ray.EndPositionWorld;
                    hits++;
                }
            }
            ...
            {
                _hasSeenEnemy = true;
                averageDirection /= hits;
                _lastEnemyPosition = averageDirection;
    ...
    public override void CollectObservations(VectorSensor sensor)
    {
        ...
        sensor.AddObservation(_hasSeenEnemy);
        
        if (_hasSeenEnemy)
        {
            var distanceToEnemy = Vector3.Distance(transform.position, _lastEnemyPosition);
            var enemyDirection = (_lastEnemyPosition - transform.position).normalized;
            var enemyAngle = Vector3.SignedAngle(transform.forward, enemyDirection, Vector3.up);
            sensor.AddObservation(distanceToEnemy / 60f);
            sensor.AddObservation(enemyAngle / 180f);
        }
        else
        {
